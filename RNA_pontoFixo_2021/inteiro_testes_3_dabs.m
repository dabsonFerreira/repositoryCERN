...
%     y1(i) = 1;
% end
%% o que falta agora é confirmar se todos os valores anteriores bateram, pois a saída não bateu e não faz sentido a menos q a culpa seja do bias
%  ver = saida - saidadabson;%pq n é constante?
%  plot(ver);grid;
%  cont = 0;
% for i = 1:length(saida)
%     for b = 1:length(saidadabson)
%         if saida(i) == saidadabson(b)
%             cont = cont +1;
%         end
%     end        
% end
% figure;stem(saidadabson);grid;title('saidamatlab, saidaquartus');hold on;
% stem(saida,'*');grid;hold off;
% figure;stem(y1);grid
% figure;stem(saidadabson - saida);title('saidamatlab - saidaquartus');
% 
%  dlmwrite('saida_dabson.txt',saida);